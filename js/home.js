(function () {
  ...
  /* ===== 二级滑动视图（Hero 式顺序切换） ===== */
  var svSlides = document.querySelectorAll('.sv-slide');
  if (svSlides.length > 1) {
    var svCur = 0;
    var svPrev = document.getElementById('svPrev');
    var svNext = document.getElementById('svNext');
    var svDots = document.querySelectorAll('#svDots span');
    var svCounter = document.getElementById('svCounter');
    var svLock = false;
    /* URL 参数 ?n=i 定位初始 slide */
    try {
      var svStart = parseInt(new URLSearchParams(location.search).get('n'), 10);
      if (!isNaN(svStart) && svStart >= 0 && svStart < svSlides.length) svCur = svStart;
    } catch (e) {}
    function svGo(i) {
      if (svLock) return;
      svLock = true;
      svCur = (i + svSlides.length) % svSlides.length;
      svSlides.forEach(function (s, k) { s.classList.toggle('on', k === svCur); });
      svDots.forEach(function (d, k) { d.classList.toggle('on', k === svCur); });
      if (svCounter) svCounter.textContent = (svCur + 1) + ' / ' + svSlides.length;
      setTimeout(function () { svLock = false; }, 550);
    }
    svGo(svCur);
    if (svPrev) svPrev.addEventListener('click', function () { svGo(svCur - 1); });
    if (svNext) svNext.addEventListener('click', function () { svGo(svCur + 1); });
    svDots.forEach(function (d, k) { d.addEventListener('click', function () { svGo(k); }); });
    document.addEventListener('keydown', function (e) {
      if (e.key === 'ArrowLeft') { svGo(svCur - 1); }
      if (e.key === 'ArrowRight') { svGo(svCur + 1); }
    });
    var svTx = 0;
    var svView = document.getElementById('svView');
    if (svView) {
      svView.addEventListener('touchstart', function (e) { svTx = e.touches[0].clientX; }, { passive: true });
      svView.addEventListener('touchend', function (e) {
        var dx = e.changedTouches[0].clientX - svTx;
        if (Math.abs(dx) > 46) svGo(svCur + (dx < 0 ? 1 : -1));
      }, { passive: true });
    }
  }
