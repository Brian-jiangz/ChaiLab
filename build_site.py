...
def tiles_body(lang, href_base='#'):
    if lang == EN:
        items = [
            'Marine Carbon Cycle and Climate Modeling',
            'Ocean Digital Twin',
            'Paleoclimate and Paleoceanography',
            'Ocean Observation and Ecosystems',
            'Marine Heatwaves and Mode Water',
        ]
        descs = [
            ...
            'Monitoring high-frequency variations of nutrients, oxygen, pH, and particulate organic carbon flux with Biogeochemical-Argo floats; assessing the impacts of climate change on fisheries resources.',
        ...
        descs = [
            '发展并改进 CESM-CoSiNE 等海洋生态—生物地球化学耦合模式，研究海洋对大气 CO₂ 的调控作用、生物泵效率及海洋碳循环对未来气候变化的响应与反馈。',
...
MP_NOTES = {
    'jiang-zheng': {
        'zh': {
            ...
            'exp': [
                ('2021–2022', '国家级大学生创新创业训练计划精品项目（优秀结题，负责人）', '项目：日本核污水排放入海对中国周边海域长时间尺度的影响。'),
                ('2020–2021', '国家级大学生创新创业训练计划精品项目（优秀结题，负责人）', '项目：新型冠状病毒传播的环境影响因子研究。'),
            ],
            'awards': [
                '中国地质大学（武汉）第13期"李四光计划"成员',
                '中国科学院大学三好学生',
                '美国大学生数学建模竞赛 Meritorious Winner（全球前 6%）',
                'Mathorcup 数学建模大赛全国三等奖',
                '全国大学生数学建模大赛湖北省三等奖',
                '"挑战杯"大学生创业计划大赛省级银奖（负责人）',
            ],
            'conf': [
        ...
        'en': {
            ...
            'exp': [
                ('2021\u20132022', 'National Undergraduate Innovation Program (Excellent completion, leader)', 'Project: Long-term impacts of Japanese nuclear wastewater discharge on Chinese marginal seas.'),
                ('2020\u20132021', 'National Undergraduate Innovation Program (Excellent completion, leader)', 'Project: Environmental drivers of COVID-19 transmission.'),
            ],
            'awards': [
                'Member of the 13th "Li Siguang Program", China University of Geosciences (Wuhan)',
                'Merit Student, University of Chinese Academy of Sciences',
                'Meritorious Winner, Mathematical Contest in Modeling (MCM) (top 6% worldwide)',
                'National Third Prize, Mathorcup Mathematical Modeling Competition',
                'Third Prize in Hubei Province, National College Mathematical Modeling Competition',
                'Provincial Silver Award, "Challenge Cup" Entrepreneurship Competition (leader)',
            ],
            'conf': [
        ...
        if profile:
            ...
            def _kv_block(key, zh, en, label):
                ...
                def _line_html(it):
                    if len(it) == 3:
                        y, org, desc = it
                        return f'        <li><span class="mp-y">{y}</span><div><b>{org}</b><span class="mp-desc">{desc}</span></div></li>'
                    y, t = it
                    return f'        <li><span class="mp-y">{y}</span><div>{t}</div></li>'
                lis = '\n'.join(_line_html(it) for it in items)
                return f'''    <section class="mp-block">
      <h3>{label}<span class="mp-en">{'' if lang == EN else en}</span></h3>
      <ul class="mp-lines">
{lis}
      </ul>
    </section>'''
      ...
      <h3>{L('获奖及荣誉', 'AWARDS & HONORS')}<span class="mp-en">{'' if lang == EN else 'AWARDS & HONORS'}</span></h3>
      <ul class="mp-awards">
{aw}
      </ul>
    </section>'''
            if profile.get('conf'):
                cf = '\n'.join(f'        <li>{c}</li>' for c in profile['conf'])
                extra += f'''    <section class="mp-block">
      <h3>{L('学术会议', 'CONFERENCES')}<span class="mp-en">{'' if lang == EN else 'CONFERENCES'}</span></h3>
      <ul class="mp-awards">
{cf}
      </ul>
    </section>'''
...
RESEARCH_ITEMS_ZH = [
    ('海洋碳循环与气候模拟', '发展并改进 CESM-CoSiNE 等海洋生态—生物地球化学耦合模式，研究海洋对大气 CO₂ 的调控作用、生物泵效率及海洋碳循环对未来气候变化的响应与反馈。'),
    ('海洋数字孪生', '构建海洋数字孪生系统，赋能蓝色经济创新，服务海洋观测—模拟—预测一体化。'),
    ('古气候与古海洋模拟', '利用地球系统模式开展末次间冰期等关键时期古气候模拟，理解碳循环的长期演化。'),
    ('海洋观测与生态系统', '利用 Biogeochemical-Argo 剖面浮标观测营养盐、溶解氧、pH 与颗粒有机碳通量的高频变化；结合生态模型评估气候变化对渔业资源分布与可持续性的影响。'),
    ('海洋热浪与模态水', '研究海洋热浪与副热带模态水的发生机制、演变规律及其对海洋生态系统与碳循环的影响。'),
]
RESEARCH_ITEMS_EN = [
    ('Marine Carbon Cycle and Climate Modeling', 'Developing and improving coupled marine ecosystem\u2013biogeochemistry models such as CESM-CoSiNE to quantify the ocean\u2019s role in regulating atmospheric CO\u2082, biological pump efficiency, and the response and feedback of the marine carbon cycle to climate change.'),
    ...
    ('Ocean Observation and Ecosystems', 'Monitoring high-frequency variations of nutrients, oxygen, pH, and particulate organic carbon flux with Biogeochemical-Argo floats; assessing the impacts of climate change on fisheries resources using ecological models and observations.'),
...
SUB_MENUS = {
    ...
    'research': [
        ('research-r0.html', '海洋碳循环与气候模拟', 'Carbon Cycle & Climate Modeling'),
